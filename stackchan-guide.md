...
> ## ⚠️ 开工前先看这条（涉封号）
>
> **连机器人的配网热点、做热点配对时，用的设备上不要登着任何 Claude。** 这条**来自其他用户的提示**（读者 [@BrainHeart5](https://x.com/BrainHeart5) 的实战报告：登着 Claude 的设备做配对，账号**秒封**）。**我们的环境不在这个风险画像里**（人在海外直连、不经代理），也因此没法复现；机制推测=**经代理/VPN 访问 Claude 的环境**里，配对时设备在几张网之间横跳、代理出口跟着跳，登着的客户端带凭证反复重试，撞上风控——未经官方证实。走代理线的读者更需警惕；封号很难挽回，按最坏情况防：
>
> - 配对用一台**没有任何 Claude 登录的干净设备**（旧手机/平板都行）
> - 出门配对最好两台干净设备：一台开热点、一台连配网热点
> - agent 只能从自己跑着的机器操作时：默认路由钉在另一张网卡上，绝不让 Claude 流量路过机器人的 AP
> - 目前的报告都发生在「连着配网 AP 折腾」那一段；配好之后正常使用未见涉及
...
- **SSID 上限 32 字节，超了自动砍短——砍短的名字能用。** 实测**满格中文热点名照扫照连**（11 个汉字被砍到 10 个，固件照存照用），列表里出现什么选什么，不用改手机名。万一真扫不到，把名字改短是排障时第一个该试的（有读者反馈简单名字更好扫，我们没遇到）。
