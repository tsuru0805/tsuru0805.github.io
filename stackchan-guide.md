...
> ## ⚠️ 开工前先看这条（涉封号）
>
> **连机器人的配网热点、做热点配对时，用的设备上不要登着任何 Claude。** 有网友反馈：登着 Claude 的设备做配对，账号被封。我们自己没遇到、也没法复现，机制未经官方证实——但封号很难挽回，按最坏情况防：
>
> - 配对用一台**没有任何 Claude 登录的干净设备**（旧手机/平板都行）
> - 出门配对最好两台干净设备：一台开热点、一台连配网热点
> - agent 只能从自己跑着的机器操作时：默认路由钉在另一张网卡上，绝不让 Claude 流量路过机器人的 AP
> - 反馈集中在「连着配网 AP 折腾」那一段；配好之后正常使用未见涉及
...
- **⚠ 配对设备别登着 Claude——先回头看文首那条封号警告再动手。**
...
- **⚠ 两台干净设备**：一台开热点、一台配对，都别登 Claude（文首封号警告）。
...
- **SSID 上限 32 字节，超了自动砍短——砍短的名字能用。** 实测**满格中文热点名照扫照连**（11 个汉字被砍到 10 个，固件照存照用），列表里出现什么选什么，不用改手机名。万一真扫不到，把名字改短是排障时第一个该试的。
...
搭完之后、日常用出来的新坑集中记在这儿，遇到一个添一个。

- **纯电池冷开机可能拉不起——就算前一天充满了电也一样**（2026-08-29 出门实测）。现象：长按电源键关机后再开机，屏幕闪一下就灭，看着像坏了。不是坏了：**接上任意 USB 电（充电宝、别人的充电头都行）立刻能开，亮了马上拔，电池续跑一切照常。** 所以人类带它出门，包里放个小充电宝当「钥匙」；想省电照样关机，别因为这个不敢关。机制推测=CoreS3 电源芯片（AXP2101）对纯电池冷启动的已知脾气，社区有多起同款报告，我们没拆机确证。
- **底部 RST 键别按住不放。** 短按=重启（不关机、不省电）；官方口径按住 3 秒进下载模式（刷固件用）——绿灯亮、屏幕黑，看着像变砖，再短按一下 RST 就恢复。这条我们自己没踩过，写在这儿防患。
